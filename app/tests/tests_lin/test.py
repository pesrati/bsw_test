...
async def test_update_test(ac_line: AsyncClient):
    response = await ac_line.patch(
        "/event/update/{event_id}?id=1&state=N", data={"id": 1, "state": "N"}
    )
    print(response.text)
    assert response.status_code == 200
...
async def test_update_test_failed(ac_line: AsyncClient):
    response = await ac_line.patch(
        "/event/update/{event_id}?id=3&state=W", data={"id": 3, "state": "W"}
    )
    print(response.text)
    assert response.status_code == 404
...
@pytest.mark.parametrize(
    "id, coefficient, deadline, state, status_code",
    [
...
async def test_create_event(
    id, coefficient, deadline, state, status_code, ac_line: AsyncClient
):
    response = await ac_line.put(
        "/event",
        json={
            "id": id,
